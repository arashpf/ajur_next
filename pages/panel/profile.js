...
import { Phone, Edit, Verified, Share } from "@mui/icons-material";
    ...
    {multiline ? (
      <textarea
        ...
        style={{
          ...
          textAlign: "right", // Added for better justification
          direction: "rtl", // Added for better justification
        }}
        onFocus={(e) => {
...
const Profile = () => {
  ...
  const handleShare = () => {
    // Share functionality - can be customized
    if (navigator.share) {
      navigator.share({
        title: ` در آجر ${name} ${family} - پروفایل و فایل های ملکی `,
        text: `پروفایل ${name} ${family} در آجر`,
        url: window.location.href,
      });
    } else {
      // Fallback for browsers that don't support Web Share API
      navigator.clipboard.writeText(window.location.href);
      setProblem("لینک کپی شد");
      set_alert_type("success");
      setOpenAlert(true);
    }
  };
              ...
              >
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "center", gap: "4px", justifyContent: "flex-end" }}>
                    <Typography variant="h4" sx={{ fontWeight: "bold", textAlign: "justify" }}>
                      {name} {family}
                    </Typography>
                  </Box>
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: "4px", justifyContent: "flex-end" }}>
                    ...
                    <Typography variant="h6" sx={{ color: "white", fontWeight: "bold", textAlign: "justify" }}>
                ...
                {/* عکس پروفایل با تیک آبی */}
                <Grid item xs="auto" sx={{ display: "flex", justifyContent: "flex-end", position: "relative" }}>
                  <Box sx={{ position: "relative", display: "inline-block" }}>
                    <ProfilePicker 
                      img={realstate.profile_url} 
                      size={130} // Increased by 30% (from 100 to 130)
                      sx={{ 
