...
import { IoSettingsOutline, IoSearch, IoClose, IoCloudOffline } from 'react-icons/io5';
...
const AsyncStorage = {
  ...
  removeItem: async (k) => typeof window !== 'undefined' ? localStorage.removeItem(k) : null,
};

// Web helpers and shims for previously React Native APIs
const showAlert = (title, message, opts = {}) => {
  const text = title + (message ? '\n' + message : '');
  if (typeof window !== 'undefined' && typeof window.__showSnackbar === 'function') {
    // map opts.severity if provided, otherwise use 'info' or 'error' heuristics
    const severity = opts.severity || (title && title.toLowerCase().includes('خطا') ? 'error' : (title && title.toLowerCase().includes('هشدار') ? 'warning' : 'success'));
...
const openSettings = () => {
  ...
  showAlert('راهنما', 'برای مدیریت دسترسی‌ها، از تنظیمات سیستم یا مرورگر استفاده کنید');
};
...
const Contacts = {
  ...
  addContact: async () => { throw new Error('Contacts not supported on web'); },
};
...
const NoteBook = () => {
  ...
  const handleDeleteNote = async (id) => {
    try {
      ...
      const success = data.success === true || data.status === 'success' || data?.deleted;
      ...
      const serverMsg = error?.response?.data?.message || error?.message || 'حذف مخاطب با مشکل مواجه شد';
      if (error?.code === 'ECONNABORTED' || error?.message?.includes('timeout')) {
        Alert.alert('خطا', 'درخواست حذف با زمان‌بندی خاتمه یافت. دوباره تلاش کنید');
  ...
  const renderHeader = () => {
      ...
      return (
        <div style={styles.searchHeader}>
          <input
            ...
            autoFocus
          />
          <button onClick={handleSearchCancel} style={{ background: 'transparent', border: 'none' }}><IoClose /></button>
        </div>
      );
    }
    ...
    return (
      <div style={styles.header}>
        <button onClick={() => setShowAboutModal(true)} style={{ background: 'transparent', border: 'none' }}><IoSettingsOutline /></button>
        <div style={styles.headerTitle}>دفترچه تلفن</div>
        <button onClick={handleSearch} disabled={!userToken} style={{ background: 'transparent', border: 'none' }}><IoSearch color={userToken ? '#5d4037' : '#ccc'} /></button>
      </div>
    );
  };
  ...
  <div style={styles.notebookContainer}>
        ...
        {isLoading && !refreshing ? (
          ...
          <div style={styles.centerContainer}>
            ...
            <div style={styles.errorText}>{userToken ? 'خطا در دریافت مخاطبین' : 'لطفا ابتدا وارد شوید'}</div>
            <button style={styles.retryButton} onClick={loadContacts}><span style={styles.retryButtonText}>تلاش مجدد</span></button>
          </div>
        ) : (
          <div style={styles.notesList}>
            ...
            {filteredNotes.length === 0 && !hasError && (
              <EmptyState isSearching={!!searchQuery.trim()} searchQuery={searchQuery} />
            )}
          </div>
        )}
      </div>
