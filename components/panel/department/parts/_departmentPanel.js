...
const DepartmentPanel = (props) => {
    ...
    if (cl) {
      return (
        ...
        >
          ...
          >
            <p style={{ fontSize: "10px", padding: "1px 10px" }}>
              <Button
                ...
                startIcon={
                  <CallIcon style={{ color: "green", fontSize: "14px" }} />
                }
              >
                تماس
              </Button>
          ...
          >
            <Link href={`/realestates/${cl.user_id}?slug=${cl.name}`}>
              <p style={{ marginTop: 10 }}>
                {cl.name} {cl.family} ({cl.worker_amount} فایل)
              </p>
            </Link>
            <p style={{ marginTop: 10 }}>
              {" "}
  ...
  const renderDepartmentWorkers = () => {
    return department_workers.map((worker) => (
      <Grid item md={6} xs={12} key={worker.id}>
        {/* <WorkerCard key={worker.id} worker={worker} /> */}
        <DepartmentWorkerCard key={worker.id} worker={worker} />
